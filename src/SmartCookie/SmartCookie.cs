...
namespace CalbucciLib
{
	...
	public class SmartCookie
	{
		...
		/// It can be overriden with SmartCookieSettings attribute by setting the Domain property.
		/// </summary>
		public string DefaultDomain { get; set; }
		/// <summary>
		/// Default Expire value for the cookie. 
		/// It can be overriden with SmartCookieSettings attribute by setting the ExpiresInDays value.
		/// </summary>
		...
		/// It can be overriden with SmartCookieSettings attribute by setting the Secure value.
		/// </summary>
		...
		private static ConcurrentDictionary<Type, Dictionary<string, SmartCookieSettingsAttribute>> _CookieSettings;

		static SmartCookie()
		{
			_CookieSettings = new ConcurrentDictionary<Type, Dictionary<string, SmartCookieSettingsAttribute>>();
			AutoTruncateCookieValueLength = 2048;
			AutoTruncateCookieMultiValueLength = 512;
		}

		public SmartCookie(HttpContextBase context)
		{
			Context = context;
		}
		...
		protected T GetMultiValue<T>([CallerMemberName] string propertyName = "") where T : SmartCookieMultiValue, new()
		{
			...
			{
				_MultiValuesCookies = new List<SmartCookieMultiValue>();
			}

			var multiValueCookie = new T { SmartCookie = this, PropertyName = propertyName };
		...
		static private Dictionary<string, SmartCookieSettingsAttribute> ProcessTypeSettings(Type type)
		{
			...
			{
				var settings =
					property.GetCustomAttribute(typeof(SmartCookieSettingsAttribute)) as
						SmartCookieSettingsAttribute;
				if (settings == null)
					continue;
				...
				if (isSmartCookieMultiValue)
				{
					...
					if (settings.ExpiresInDays > 0 || settings.Secure || settings.Domain != null || settings.HttpOnly ||
						settings.Path != null)
					{
						Trace.WriteLine("Type " + type + " define SmartCookieSettings elements that will be ignored.");
					}
				}
			}
		...
		private SmartCookieSettingsAttribute GetPropertySettings(string propertyName, Type type = null)
		{
			if (type == null)
			{
				type = GetType();
			...
			Dictionary<string, SmartCookieSettingsAttribute> typeSettings;
			...
			SmartCookieSettingsAttribute settings;
		...
		private string ConvertPropertyNameToCookieName(string propertyName)
		{
			...
			SmartCookieSettingsAttribute setting = GetPropertySettings(propertyName);
			if (setting != null)
			{
		...
		private Tuple<string, int> ConvertValuePropertyNameToCookieValueName(Type multiValueType, string valuePropertyName)
		{
			...
			SmartCookieSettingsAttribute settingSubvalue = GetPropertySettings(valuePropertyName, multiValueType);
